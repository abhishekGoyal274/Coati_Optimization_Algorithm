...
struct COAResult {
    double best_score;
    vector<double> best_iguana;
    COAResult(double score, int initialSize) : best_score(score), best_iguana(initialSize) {} 
};
...
COAResult COA_OpenMP(int F, int Coatis, int Max_iterations) {
        ...
        for(int i=0; i<Coatis/2; i++){
            ...
            #pragma omp critical
            {
                if(fitness_new < result.best_score){
                    ...
                    result.best_iguana = coati_new_position;
                }
            }
...
int main(int argc, char** argv) {
    ...
    cout << "COA OpenMP, F" << F << ", Coatis=" << Coatis << ", Max_iterations=" << Max_iterations << "\n";

    auto start = std::chrono::high_resolution_clock::now();
    auto result = COA_OpenMP(F, Coatis, Max_iterations);
    ...
    for (int i = 0; i < result.best_iguana.size(); i++)
        cout << result.best_iguana[i] << " ";
    cout << endl;
